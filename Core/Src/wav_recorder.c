...
FIL WavFile; // Plik , domyslnie w formacie .wav// nie wiem czym ejst FIL, w sensie czy domyslnie generowana struktuta czy dodana
...
void WavRecordingProcess(uint8_t recordNumber) {
	...
	/*if (BSP_AUDIO_IN_Init(DEFAULT_AUDIO_IN_FREQ,
	 ...
	 Info_UART(
	 "NIE UDALO SIE ZAINICJALIZAOWAC AUDIO_IN\ (wav_recorder.c)r\n");
	 else
...
// Zostanie nadpisany po nagraniu wiadomosci
	header[4] = 0x00;
	header[5] = 0x4C; //// czy tu musza byc te wartosci?
	header[6] = 0x1D;
	header[7] = 0x00;
	...
	// Liczba kanalow
	header[22] = 0x02; // 2 kanaly, nie powinien byc 1?
	header[23] = 0x00;
	...
	//uint32_t bytesPerSecond = (NUMBER_OF_CHANNELS * AUDIO_IN_FREQUENCY * AUDIO_RESOLUTION) / 8;
	uint32_t bytesPerSecond = (AUDIO_IN_FREQUENCY * (AUDIO_RESOLUTION / 8) *
	NUMBER_OF_CHANNELS);
